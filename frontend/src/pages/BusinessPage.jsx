...
import { BarChart3, ReceiptText, Bell } from 'lucide-react';
...
export const BusinessPage = () => {
  ...
  return <div className="space-y-6"><h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2"><Bell className="w-6 h-6 text-indigo-400" /> Notifications</h1><div className="space-y-3">{notifications.map((notification) => <div key={notification._id} className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-slate-100">{notification.title}</p><p className="text-sm text-slate-400 mt-1">{notification.message}</p></div><Badge variant={notification.read ? 'default' : 'brand'}>{notification.read ? 'Read' : 'New'}</Badge></div><p className="text-xs text-slate-500 mt-3">{formatDate(notification.createdAt)}</p></div>)}{notifications.length === 0 && <p className="text-sm text-slate-500">No notifications available yet.</p>}</div></div>;
};